...
class OperacoesObservacoes:
    ...
    def adicionar_od(self, num_int, nome, responsavel, data, tipo, repreensao, descricao):
        ...
        nova_od = ObservacoesDinamicas(Num_interno=num_int, Nome=nome.upper(), Responsavel=responsavel, Data=data, TipoDeOD=tipo, Repreensao=repreensao, Descricao=descricao)
        self.session.add(nova_od)
        self.session.commit()
        self.session.close()

    def buscar_ods_aspirante(self, nome, num_int):
        df = pd.DataFrame(columns=['Numero Interno', 'Nome', 'Responsavel', 'Data','Tipo de OD', 'Repreensao?', 'Descricao'])
        if (nome is None  or nome == '') and (num_int is not None and num_int != ''):
            nome = CorpoAspirantes().buscar_aspirante('Num_interno', num_int).Nome
        elif (num_int is None or num_int == '') and (nome is not None and nome != ''):
            num_int = CorpoAspirantes().buscar_aspirante('Nome', nome.upper()).Numero
        ...
        for od in query:
            df_ = pd.DataFrame({'Numero Interno': od.Num_interno, 'Nome': od.Nome, 'Responsavel': od.Responsavel, 'Data': od.Data, 'Tipo de OD': od.TipoDeOD, 'Repreensao?': od.Repreensao, 'Descricao': od.Descricao}, index=[od.id])
